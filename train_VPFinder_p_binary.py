...
best_f1_path = "model_best/VPFinder_p_binary_f1.txt"
best_model_path = "model_best/VPFinder_p_binary_model.pth"
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return batch_del, batch_add, batch_y
...
class VulClassifier(nn.Module):
    ...
    def forward(self, del_id, add_id, del_mask, add_mask):
        ...
        output1 = torch.cat((output_del, output_add), dim=1)
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    for epoch in range(epochs):
        ...
        for delete, add, labels in tqdm(train_dataloader):
        ...
        with torch.no_grad():
            for delete, add, labels in tqdm(val_dataloader):
                del_ids = delete['input_ids'].squeeze(1).to(device)
                add_ids = add['input_ids'].squeeze(1).to(device)
                del_mask = delete['attention_mask'].squeeze(1).to(device)
                add_mask = add['attention_mask'].squeeze(1).to(device)

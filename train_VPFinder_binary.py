...
class VulClassifier(nn.Module):
    ...
    def forward(self, input_id, del_id, add_id, message_id, mask, del_mask, add_mask, message_mask):
        pooled_output1 = self.bert2(input_ids=input_id, attention_mask=mask, return_dict=False)[0]
        output_text = pooled_output1[:, 0, :]
        pooled_output2 = self.bert1.roberta(input_ids=del_id, attention_mask=del_mask, return_dict=False)[0]
        output_del = pooled_output2[:, 0, :]
        pooled_output3 = self.bert1.roberta(input_ids=add_id, attention_mask=add_mask, return_dict=False)[0]
        output_add = pooled_output3[:, 0, :]
        pooled_output4 = self.bert2(input_ids=message_id, attention_mask=message_mask, return_dict=False)[0]
        output_message = pooled_output4[:, 0, :]
        output1 = torch.cat((output_text, output_del, output_add, output_message), dim=1)
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    for epoch in range(epochs):
        ...
        for texts, delete, add, messages, labels in tqdm(train_dataloader):
        ...
        with torch.no_grad():
            for texts, delete, add, messages, labels in tqdm(val_dataloader):
                input_ids = texts['input_ids'].squeeze(1).to(device)
                del_ids = delete['input_ids'].squeeze(1).to(device)
                add_ids = add['input_ids'].squeeze(1).to(device)
                message_ids = messages['input_ids'].squeeze(1).to(device)

...
best_f1_path = "model_best/VPFinder_m_binary_f1.txt"
best_model_path = "model_best/VPFinder_m_binary_model.pth"
...
class VulClassifier(nn.Module):
    ...
    def forward(self, message_id, message_mask):
        pooled_output = self.bert2(input_ids=message_id, attention_mask=message_mask, return_dict=False)[0]
        output_message = pooled_output[:, 0, :]
        # need new compare method
        output = self.hidden(output_message)
        return output
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for messages, labels in tqdm(val_dataloader):
                message_ids = messages['input_ids'].squeeze(1).to(device)
                message_mask = messages['attention_mask'].squeeze(1).to(device)
                labels = labels.long().to(device)
